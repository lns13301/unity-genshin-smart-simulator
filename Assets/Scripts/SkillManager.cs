...
public class SkillManager : MonoBehaviour
{
    ...
    public void OnStatUI(Item item)
    {
        ...
        if (item.type == ItemType.CHARACTER)
        {
            ...
            statSet.transform.GetChild(4).gameObject.SetActive(true);

            if (LanguageManager.instance.language == Language.KOREAN)
            {
                statSet.transform.GetChild(4).GetChild(0).GetComponent<Text>().text = "운명의 자리";
            }
            else
            {
                statSet.transform.GetChild(4).GetChild(0).GetComponent<Text>().text = "Constellation";
            }
        }
        else
        {
            ...
            if (LanguageManager.instance.language == Language.KOREAN)
            {
                statSet.transform.GetChild(4).GetChild(0).GetComponent<Text>().text = "파괴";
            }
            else
            {
                statSet.transform.GetChild(4).GetChild(0).GetComponent<Text>().text = "Destroy";
            }
        }
